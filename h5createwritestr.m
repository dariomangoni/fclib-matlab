...
try 
    ...
    for i = 2:length(slashes) 
        ...
        try 
        H5G.create(file, url, 1024);%1024 "specifies the number of bytes to reserve for the names that will appear in the group" 
        catch	
        end 
    end 
    ...
    %create a dataset plist for chunking. (A dataset can't be unlimited unless the chunk size is defined.) 
